...
ArchwayVALIDATORADDRESS="" #if left empty default is from status call (Archway validator)
...
nmsg_synced="Your Archway node is now in synced"
nmsg_unsynced="Your Archway node is no longer in synced"

#node stuck
lastblockheight=0
node_stuck_n="false" # true or false indicating the notification state of a node stuck
nmsg_nodestuck="Your Archway node is now stuck"
nmsg_node_no_longer_stuck="Your Archway node is no longer stuck, Yeah !"
node_stuck_status="NA" #node stucktest status to print out to log file

#Archway validator run (Archwayd)
Archwayd_run_n="true" # true or false indicating whether Archwayd is running or not
nmsg_Archwayd_run_ok="$HOSTNAME: Your Archwayd is running ok now"
nmsg_Archwayd_run_nok="@here $HOSTNAME: Your Archwayd has just stop running, fix it !"
Archwayd_run_status="NA" #Archwayd test status to print out to log file 
...
if [ -z $CONFIG ]; then
        CONFIG=~/.archwayd/config/config.toml;
fi
...
then 
        ...
        ArchwayVALIDATORADDRESS=$(echo $KEYRINGPW | archwayd keys show $NAME --bech val -a);
...
echo "validator address: $ArchwayVALIDATORADDRESS"
...
echo "$date status=scriptstarted chainid=$chainid" >>$logfile
    ...
    # Checking Archwayd process running
    if pgrep archwayd >/dev/null; then
        echo "Is Archwayd binary running: Yes";
        Archwayd_run_status="OK"
        if [ $Archwayd_run_n == "false" ]; then #Archwayd process was not ok
        send_notification "$nmsg_Archwayd_run_ok"
        Archwayd_run_n="true"
        fi
    else
        echo "Is Archwayd binary running: No, please restart it;"
        Archwayd_run_status="NOK"
        if [ $Archwayd_run_n == "true" ]; then #Archwayd process was ok
        send_notification "$nmsg_Archwayd_run_nok"
        Archwayd_run_n="false"
        fi
    fi
