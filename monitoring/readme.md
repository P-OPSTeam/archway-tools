# Monitoring Archway node

To have automatic monitoring of your Archway Node & Validator enabled one can follow this guide.

## Script nodemonitor.sh

To monitor the status of the Archway Node & Validator it's possible to run the script **nodemonitor.sh** available in this repository.
...
* you have the script placed in your **_$HOME/archway-tools/monitoring_** directory
* run chmod +x /home/$USER/archway-tools/monitoring/nodemonitor.sh
...
Create a file called **Archway-nodemonitor.service** in the **~/.config/systemd/user/** by following the commands:

```bash
mkdir -p ~/.config/systemd/user
cat<<-EOF > ~/.config/systemd/user/Archway-nodemonitor.service
[Unit]
Description=Archway NodeMonitor
...
Restart=always
RestartSec=1
ExecStart=/bin/bash -c '. "\$0" && exec "\$@"' /home/$USER/.profile /home/$USER/archway-tools/monitoring/nodemonitor.sh
...
```bash
systemctl --user start Archway-nodemonitor
```

To make sure the service will be active even when a reboot takes place, use:

```bash
systemctl --user enable Archway-nodemonitor
```

Check the status of the service with:

```bash
systemctl --user status Archway-nodemonitor
```
...
```bash
journalctl --user -fu Archway-nodemonitor
```
...
git pull
git stash pop
systemctl --user stop Archway-nodemonitor
systemctl --user start Archway-nodemonitor
```
